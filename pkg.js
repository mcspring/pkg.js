/* Copyright (c) 2012 Spring Mc <Heresy.Mc@gmail.com>. MIT Licensed */
// This is forked from https://github.com/chriso/load.js
...
addMethod('require', function (args, argc) {
    for (var q = [], i = 0; i < argc; i++) {
        (function (i) {
            q.push(function (next, error) {
                js_require(args[i], next, error);
            });
        }(i));
    }
    this.call('run', q);
...
addMethod('rely', function (args, argc) {
    for (var q = [], i = 0; i < argc; i++) {
        i += (function (i) {
            var obj = 'undefined';
            try {
                obj = eval('typeof ' + args[i]);
            } catch (e) {}

            q.push(function (next, error) {
                js_require((obj === 'undefined') ? args[i+1] : null, next, error);
            }
        }(i));
    }
    this.call('run', q);
...
function js_require(src, onload, onerror) {
    if (src === null) return onload();
